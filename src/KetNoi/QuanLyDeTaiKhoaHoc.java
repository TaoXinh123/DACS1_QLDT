...
public class QuanLyDeTaiKhoaHoc extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					QuanLyDeTaiKhoaHoc frame = new QuanLyDeTaiKhoaHoc();
...
//		panel_3.add(table_1);
			...
			public void actionPerformed(ActionEvent e) {
				if(JOptionPane.showConfirmDialog(null, "Bạn muốn thêm ?")==JOptionPane.YES_OPTION) {
					try {
						...
						//con.addDeTai(maDeTai, tenDeTai, linhVuc, ngayNop);// them vo sql
		...
		updatedt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(JOptionPane.showConfirmDialog(null, "Bạn thật sự muốn sửa ?")==JOptionPane.YES_OPTION) {
					...
					//	con.Updatedt(maDeTai, tenDeTai, linhVuc, ngayNop);// them vo sql
		...
		tabledt.setModel(new DefaultTableModel(
			...
			new String[] {
				"Mã đề tài", "Tên đề tài", "Lĩnh vực", "Ngày nộp"
			}
		));
